...
def main(base_dir_path=None):
    ...
    if args.device is None:
        import _utils_gpu
        device = _utils_gpu.get_gpu_with_largest_available_useage()
    else:
        device = args.device
    ...
    LossLog = FloatLog(report_batch_num)
    AccLog = IntLog(report_batch_num)
    context = Dict()
    ...
    for train_epoch_index in range(args.epoch_num):
        ...
        with pipe_out.increased_indent():
            ...
            for train_batch_index in range(train_batch_num):
                ...
                LossLog.append(loss.item(), batch_size)
                AccLog.append(is_pred_correct.sum(), batch_size)
                reporter.tick()
                # pipe_out.PrintEvery(100, "TrainBatch %d"%train_batch_index)
        try:
            end = next(train_data)
